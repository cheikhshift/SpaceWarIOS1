...
-(id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) {
        ...
        SKSpriteNode *sn = [SKSpriteNode spriteNodeWithImageNamed:@"background 10.png"];
        
        sn.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame));
        sn.name = @"BACKGROUND";
        
        [self addChild:sn];
        [self initalizingScrollingBackground:@"Planet 10.png"];
        [self addShip];
        ...
        _playerlife = 50;
        _killedoff = 0;
        ennemyfleet = @{ @"k1" : @"planet 10 destroyers.png", @"k2" : @"planet 10 captains.png", @"k3" : @"planet 10 guards.png" };
...
-(void)addShip
{
    ...
    retryButton.position = CGPointMake(self.frame.size.width - 50, 10);
...
-(void)addSpecial:(NSString *) withName
{
    ...
    [self addChild:missile];
    SKAction *align;
    if ([withName isEqualToString:@"planet 10 destroyers.png"]) {
        align = [SKAction moveByX:-120 y:0 duration:1.4];
         _playerScore = _playerScore + 25;
    }
    else if ([withName isEqualToString:@"planet 10 captains.png"]){
...
-(void) addboss {
 ...
 //next level
    ...
    SKScene * nextlevel = [[levelTransition alloc] initWithSize:self.size score:_playerScore next:1 caption:@"Welcome to a rocky planet"];
...
-(void)update:(CFTimeInterval)currentTime {
    ...
    if( currentTime - _lastMissileAdded > 1)
    {
        ...
        [self addObstacle:@"planet 10 soldiers.png"];
         _playerScore = _playerScore + 1;
        _lastAttack = _lastAttack + 1;
        
        if(_currentRendered < 7){
            if(_lastAttack == 5){
        ...
        (secondBody.categoryBitMask & obstacleCategory) != 0)
    {
       ...
       // SKScene * gameOverScene = [[GameOverScene alloc] initWithSize:self.size score:player_score];
        ...
        if(_playerlife > 0){
            ...
            SKTransition *reveal = [SKTransition flipHorizontalWithDuration:0.5];
            SKScene * gameOverScene = [[cosmosPAUSE alloc] initWithSize:self.size score:_playerScore ifreplay:0];
            [self.view presentScene:gameOverScene transition: reveal];
        }
        //NSLog(@"%i", _playerlife);
