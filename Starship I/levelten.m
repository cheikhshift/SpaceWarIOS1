...
-(id)initWithSize:(CGSize)size score: (NSInteger)player_score{
    if (self = [super initWithSize:size]) {
        ...
        SKSpriteNode *sn = [SKSpriteNode spriteNodeWithImageNamed:@"background 9.png"];
        
        sn.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame));
        sn.name = @"BACKGROUND";
        
        [self addChild:sn];
        [self initalizingScrollingBackground:@"Planet 9.png"];
        [self addShip];
        _lastAttack = 0;
        _fileinter = 0;
        _currentRendered = 0;
        ...
        _playerlife = 50;
        _killedoff = 0;
        ennemyfleet = @{ @"k1" : @"planet 9 destroyers.png", @"k2" : @"planet 9 captains.png", @"k3" : @"planet 9 guards.png" };
...
-(void)addSpecial:(NSString *) withName
{
    ...
    [self addChild:missile];
    SKAction *align;
    if ([withName isEqualToString:@"planet 9 destroyers.png"]) {
        align = [SKAction moveByX:-90 y:0 duration:1.4];
        _playerScore = _playerScore + 25;
    }
    else if ([withName isEqualToString:@"planet 9 captains.png"]){
        align = [SKAction moveByX:-185 y:0 duration:1.4];
...
- (void)moveObstacle
{
    ...
    for(SKNode * node in nodes){
        if([node.name isEqualToString:@"LASER"]){
            ...
            SKAction *moveup = [SKAction moveByX:0 y:1 duration:1.3];
            SKAction *movedown = [SKAction moveByX:0 y:-1 duration:1.3];
    ...
    if(anyleft == 0 && _bossLife == 0){
        ...
        SKScene * nextlevel = [[levelTransition alloc] initWithSize:self.size score:_playerScore next:10 caption:@"Welcome to a rocky planet"];
...
-(void) addboss {
    ...
    if(_killedoff < 16){
        ...
        SKSpriteNode *missile;
        missile = [SKSpriteNode spriteNodeWithImageNamed:@"planet 9 boss.png"];
        [missile setScale:0.40];
        //missile.zRotation =  M_PI / 2;
        //Adding SpriteKit physicsBody for collision detection
        missile.physicsBody = [SKPhysicsBody bodyWithRectangleOfSize:missile.size];
        missile.physicsBody.categoryBitMask = ennemyShip;
        ...
        [self addSpecial:@"planet 9 captains.png"];
        [self addSpecial:@"planet 9 captains.png"];
         [self addSpecial:@"planet 9 destroyers.png"];
         [self addSpecial:@"planet 9 destroyers.png"];
        [missile runAction:align];
...
-(void)update:(CFTimeInterval)currentTime {
    ...
    if (self.paused == NO ) {
        ...
        if( currentTime - _lastMissileAdded > 1)
        {
            ...
            [self addObstacle:@"planet 9 soldiers.png"];
            _playerScore = _playerScore + 1;
            _lastAttack = _lastAttack + 1;
            
            if(_currentRendered < 17){
                if(_lastAttack == 4){
...
- (void)didBeginContact:(SKPhysicsContact *)contact
{
        ...
        (secondBody.categoryBitMask & obstacleCategory) != 0)
    {
        ...
        if(_playerlife > 0){
            ...
            SKTransition *reveal = [SKTransition flipHorizontalWithDuration:0.5];
            SKScene * gameOverScene = [[cosmosPAUSE alloc] initWithSize:self.size score:_playerScore ifreplay:9];
            [self.view presentScene:gameOverScene transition: reveal];
        }
        //NSLog(@"%i", _playerlife);
