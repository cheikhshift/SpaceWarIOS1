...
-(id)initWithSize:(CGSize)size score: (NSInteger)player_score{
    if (self = [super initWithSize:size]) {
        ...
        SKSpriteNode *sn = [SKSpriteNode spriteNodeWithImageNamed:@"background 2.png"];
        
        sn.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame));
        sn.name = @"BACKGROUND";
        
        [self addChild:sn];
        [self initalizingScrollingBackground:@"Planet 2.png"];
        [self addShip];
        _lastAttack = 0;
        _fileinter = 0;
        _currentRendered = 0;
        ...
        _killedoff = 0;
        _bossLife = 90;
        ennemyfleet = @{ @"k1" : @"planet 2 destroyers.png", @"k2" : @"planet 2 captains.png", @"k3" : @"planet 2 guards.png" };
...
-(void)addSpecial:(NSString *) withName
{
    ...
    [self addChild:missile];
    SKAction *align;
    if ([withName isEqualToString:@"planet 2 destroyers.png"]) {
        align = [SKAction moveByX:-100 y:0 duration:1.4];
        _playerScore = _playerScore + 25;
    }
    else if ([withName isEqualToString:@"planet 2 captains.png"]){
...
- (void)moveObstacle
{
    ...
    for(SKNode * node in nodes){
        if([node.name isEqualToString:@"LASER"]){
        ...
        else if ([node.name isEqualToString:@"LASERBOSS"]){
            ...
            //update score based on latest number
                ...
                SKAction *movedown = [SKAction moveByX:0 y:-4 duration:1.3];
                  SKAction *bite = [SKAction moveByX:0 y:2 duration:2.5];
                   SKAction *biteback = [SKAction moveByX:0 y:-2 duration:2.5];
                ...
                if(ob.position.y > 0 && ob.position.y < self.frame.size.height){
                    ...
                    SKAction *movein = [SKAction moveByX:0 y:-15 duration:1.3];
                    [ob runAction:movein];
                }
                else {
                    SKAction *movein = [SKAction moveByX:0 y:15 duration:1.3];
    ...
    if(anyleft == 0 && _bossLife == 0){
        ...
        SKScene * nextlevel = [[levelTransition alloc] initWithSize:self.size score:_playerScore next:3 caption:@"..."];
...
-(void) addboss {
    ...
    if(_killedoff < 8){
        ...
        //initalizing spaceship node
        SKSpriteNode *missile;
        missile = [SKSpriteNode spriteNodeWithImageNamed:@"planet 2 boss.png"];
        ...
        [self addChild:missile];
        SKAction *align = [SKAction moveByX:-200 y:0 duration:1.4];
        _playerScore = _playerScore + 150;
        [self addSpecial:@"planet 2 captains.png"];
        [self addSpecial:@"planet 2 captains.png"];
        [missile runAction:align];
...
-(void)update:(CFTimeInterval)currentTime {
    ...
    if (self.paused == NO ) {
        ...
        if( currentTime - _lastMissileAdded > 1)
        {
            ...
            [self addObstacle:@"planet 2 soldiers.png"];
            _playerScore = _playerScore + 1;
            _lastAttack = _lastAttack + 1;
            
            if(_currentRendered < 11){
                if(_lastAttack == 4){
...
- (void)didBeginContact:(SKPhysicsContact *)contact
{
        ...
        (secondBody.categoryBitMask & obstacleCategory) != 0)
    {
        ...
        if(_playerlife > 0){
            ...
            SKTransition *reveal = [SKTransition flipHorizontalWithDuration:0.5];
            SKScene * gameOverScene = [[cosmosPAUSE alloc] initWithSize:self.size score:_playerScore ifreplay:2];
            [self.view presentScene:gameOverScene transition: reveal];
        }
        //NSLog(@"%i", _playerlife);
