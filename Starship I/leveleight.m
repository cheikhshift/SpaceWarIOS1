...
-(id)initWithSize:(CGSize)size score: (NSInteger)player_score{
    if (self = [super initWithSize:size]) {
        ...
        SKSpriteNode *sn = [SKSpriteNode spriteNodeWithImageNamed:@"background 7.png"];
        
        sn.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame));
        sn.name = @"BACKGROUND";
        
        [self addChild:sn];
        [self initalizingScrollingBackground:@"Planet 7.png"];
        [self addShip];
        _lastAttack = 0;
        _fileinter = 0;
        _currentRendered = 0;
        ...
        _killedoff = 0;
        _bossLife = 120;
        ennemyfleet = @{ @"k1" : @"planet 7 destroyers.png", @"k2" : @"planet 7 captains.png", @"k3" : @"planet 7 guards.png" };
        ...
        movey = @{ @"k1" : @2, @"k2" : @30, @"k3" : @20 , @"k4": @0 , @"k5": @1 , @"k6": @18, @"k7": @0, @"k8": @6, @"k9": @5, @"k10": @4, @"k11": @4, @"k12": @12, @"k13": @14, @"k14": @1, @"k15": @2, @"k16": @2, @"k17": @0, @"k18": @4, @"k19": @9, @"k20": @10, @"k21": @8, @"k22": @1, @"k23": @4, @"k24": @9, @"k26": @26, @"k25": @5, @"k26": @2, @"k27": @6, @"k28": @0, @"k29": @2, @"k30": @1, @"k31": @1, @"k32": @1, @"k33": @1};
...
-(void)addObstacle:(NSString *) withName
{
    ...
    [missile setScale:0.13];
    missile.zRotation = - M_PI;
    //Adding SpriteKit physicsBody for collision detection
    missile.physicsBody = [SKPhysicsBody bodyWithRectangleOfSize:missile.size];
    missile.physicsBody.categoryBitMask = obstacleCategory;
    missile.physicsBody.dynamic = YES;
...
-(void)addSpecial:(NSString *) withName
{
    ...
    [self addChild:missile];
    SKAction *align;
    if ([withName isEqualToString:@"planet 7 destroyers.png"]) {
        align = [SKAction moveByX:-100 y:0 duration:1.4];
        _playerScore = _playerScore + 25;
    }
    else if ([withName isEqualToString:@"planet 7 captains.png"]){
        align = [SKAction moveByX:-195 y:0 duration:1.4];
        _playerScore = _playerScore + 15;
    }
    else {
        align = [SKAction moveByX:-250 y:0 duration:1.4];
...
- (void)moveObstacle
{
    ...
    for(SKNode * node in nodes){
        if([node.name isEqualToString:@"LASER"]){
            ...
            //NSLog(@"%@", movey[key]);
                ...
                switch (random) {
                    case 5:
                        ...
                        [self bossshoot:@"4A8_Fireball1_GD.png" sprite:ob];
                        break;
                    case 2:
        ...
        else if ([node.name isEqualToString:@"jaw"]){
            //update score based on latest number && do nothing bossshoot will hanfle it
            ...
            if([node.name isEqualToString:@"special"]){
                ...
                else if (ob.position.y > self.frame.size.width){
    ...
    if(anyleft == 0 && _bossLife == 0){
        ...
        SKScene * nextlevel = [[levelTransition alloc] initWithSize:self.size score:_playerScore next:8 caption:@"Welcome to a rocky planet"];
...
-(void) bossshoot:(NSString *)graphic  sprite:(SKSpriteNode *) spot {
    ...
    missile.position = CGPointMake(spot.frame.size.width + 40, spot.position.y + 55);
    
    [self enumerateChildNodesWithName:@"jaw" usingBlock: ^(SKNode *node, BOOL *stop)
     {
         SKSpriteNode * bg = (SKSpriteNode *) node;
         SKAction *rotation = [SKAction rotateByAngle:   M_PI/90.0 duration:1];
         //and just run the action
         if(bg.zRotation < 0.6){
         [bg runAction: rotation];
...
-(void) addboss {
    ...
    //next level
       if(_killedoff < 11){
        ...
        SKSpriteNode *missile;
        missile = [SKSpriteNode spriteNodeWithImageNamed:@"planet 7 boss.png"];
        [missile setScale:0.45];
        ...
        missile.position = CGPointMake(self.frame.size.width + 120, self.frame.size.height / 2 - 75);
        
        [self addChild:missile];
        
        SKSpriteNode *jaw;
        jaw = [SKSpriteNode spriteNodeWithImageNamed:@"planet7jaw.png"];
        [jaw setScale:0.45];
        //jaw.zRotation =  M_PI / 2;
        //Adding SpriteKit physicsBody for collision detection
        jaw.physicsBody = [SKPhysicsBody bodyWithRectangleOfSize:jaw.size];
        jaw.physicsBody.categoryBitMask = ennemyShip;
        jaw.physicsBody.dynamic = YES;
        jaw.physicsBody.contactTestBitMask = playerLaser;
        jaw.physicsBody.collisionBitMask = 0;
        jaw.physicsBody.usesPreciseCollisionDetection = YES;
        jaw.name = @"jaw";
        
        //selecting random y position for jaw
        
        jaw.position = CGPointMake(self.frame.size.width + 120, self.frame.size.height / 2 - 5);
        
        [self addChild:jaw];
        
        SKAction *align = [SKAction moveByX:-120 y:0 duration:1.4];
        _playerScore = _playerScore + 150;
        [self addSpecial:@"planet 7 captains.png"];
        [self addSpecial:@"planet 7 captains.png"];
         [self addSpecial:@"planet 7 destroyers.png"];
         [self addSpecial:@"planet 7 destroyers.png"];
        [missile runAction:align];
         [jaw runAction:align];
    }
...
-(void)update:(CFTimeInterval)currentTime {
    ...
    if (self.paused == NO ) {
        ...
        if( currentTime - _lastMissileAdded > 1)
        {
            ...
            [self addObstacle:@"planet 7 soldiers.png"];
            _playerScore = _playerScore + 1;
            _lastAttack = _lastAttack + 1;
            
            if(_currentRendered < 12){
                if(_lastAttack == 5){
...
- (void)didBeginContact:(SKPhysicsContact *)contact
{
        ...
        (secondBody.categoryBitMask & obstacleCategory) != 0)
    {
        ...
        if(_playerlife > 0){
            ...
            SKTransition *reveal = [SKTransition flipHorizontalWithDuration:0.5];
            SKScene * gameOverScene = [[cosmosPAUSE alloc] initWithSize:self.size score:_playerScore ifreplay:7];
            [self.view presentScene:gameOverScene transition: reveal];
        }
        //NSLog(@"%i", _playerlife);
    ...
    else
    {
        ...
        else  if([TG.name isEqualToString:@"jaw"]){
            
            if(_bossLife == 0){
            TG.name = @"BOOM";
            TG.texture = [SKTexture textureWithImageNamed:@"misc_explosion_element_png_by_dbszabo1-d54iy0g.png"];
            _playerScore = _playerScore + 100;
            [self enumerateChildNodesWithName:@"BOSS" usingBlock: ^(SKNode *node, BOOL *stop)
             {
                 SKSpriteNode * bg = (SKSpriteNode *) node;
                 [bg removeFromParent];
