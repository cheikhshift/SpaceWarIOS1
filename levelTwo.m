...
-(id)initWithSize:(CGSize)size score: (NSInteger)player_score{
    if (self = [super initWithSize:size]) {
        ...
        SKSpriteNode *sn = [SKSpriteNode spriteNodeWithImageNamed:@"backgroundscc1.png"];
        
        sn.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame));
        sn.name = @"BACKGROUND";
        
        [self addChild:sn];
        [self initalizingScrollingBackground:@"planetoo1.png"];
        [self addShip];
        _lastAttack = 0;
        _fileinter = 0;
        _currentRendered = 0;
        ...
        _killedoff = 0;
        _bossLife = 80;
        ennemyfleet = @{ @"k1" : @"planet 1 destroyers.png", @"k2" : @"planet 1 captains.png", @"k3" : @"planet 1 guards.png" };
...
-(void)addSpecial:(NSString *) withName
{
    ...
    [self addChild:missile];
    SKAction *align;
    if ([withName isEqualToString:@"planet 1 destroyers.png"]) {
        align = [SKAction moveByX:-110 y:0 duration:1.4];
        _playerScore = _playerScore + 25;
    }
    else if ([withName isEqualToString:@"planet 1 captains.png"]){
        align = [SKAction moveByX:-180 y:0 duration:1.4];
...
- (void)moveObstacle
{
        ...
        if([node.name isEqualToString:@"LASER"]){
            ...
            //update score based on latest number
                ...
                if(ob.position.y > 0 && ob.position.y < self.frame.size.height){
                    ...
                    SKAction *movein = [SKAction moveByX:0 y:-5 duration:1.3];
                    [ob runAction:movein];
                }
                else {
                    SKAction *movein = [SKAction moveByX:0 y:5 duration:1.3];
    ...
    if(anyleft == 0 && _bossLife == 0){
        ...
        SKScene * nextlevel = [[levelTransition alloc] initWithSize:self.size score:_playerScore next:2 caption:@"..."];
...
-(void) addboss {
    ...
    if(_killedoff < 9){
        ...
        //initalizing spaceship node
        SKSpriteNode *missile;
        missile = [SKSpriteNode spriteNodeWithImageNamed:@"planet 1 boss.png"];
        ...
        [self addSpecial:@"planet 1 captains.png"];
         [self addSpecial:@"planet 1 captains.png"];
        [missile runAction:align];
...
-(void)update:(CFTimeInterval)currentTime {
    ...
    if (self.paused == NO ) {
        ...
        if( currentTime - _lastMissileAdded > 1)
        {
            ...
            [self addObstacle:@"planet 1 soldiers.png"];
            _playerScore = _playerScore + 1;
            _lastAttack = _lastAttack + 1;
            
            if(_currentRendered < 10){
                if(_lastAttack == 5){
...
- (void)didBeginContact:(SKPhysicsContact *)contact
{
        ...
        (secondBody.categoryBitMask & obstacleCategory) != 0)
    {
        ...
        if(_playerlife > 0){
            ...
            SKTransition *reveal = [SKTransition flipHorizontalWithDuration:0.5];
            SKScene * gameOverScene = [[cosmosPAUSE alloc] initWithSize:self.size score:_playerScore ifreplay:1];
            [self.view presentScene:gameOverScene transition: reveal];
        }
        //NSLog(@"%i", _playerlife);
